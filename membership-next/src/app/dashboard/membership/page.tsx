...
import { 
  CheckCircle, ArrowRight, Award, Shield, Sparkles, Loader2, ChevronRight, Lock, AlertTriangle
} from "lucide-react";
...
export default function MembershipPurchasePage() {
  ...
  // Check if profile is complete enough to purchase
  ...
  const getButtonState = (tierName: "Silver" | "Gold" | "Platinum") => {
    ...
    // 3. Profile incomplete — BLOCK purchase
    ...
    // 4. Pending/Waiting verification membership invoice
  ...
  const getButtonClass = (variant: string, defaultClass: string) => {
    switch (variant) {
      ...
      case "profile-incomplete":
        return "w-full py-3 rounded-xl text-xs font-bold bg-gray-100 text-gray-400 border-2 border-dashed border-gray-300 cursor-not-allowed flex items-center justify-center gap-2";
      case "pending-this":
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      ...
      {!isProfileComplete && (
        <div className="mb-8 p-5 rounded-2xl border-2 border-orange-200 bg-orange-50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-xl bg-orange-100 flex items-center justify-center flex-shrink-0 mt-0.5">
              <Lock className="w-5 h-5 text-orange-600" />
            </div>
            <div>
              <div className="font-bold text-sm text-orange-800" style={{ fontFamily: "Montserrat, sans-serif" }}>
                🔒 Pembelian Dikunci — Lengkapi Profil Anda Terlebih Dahulu
              </div>
              <p className="text-xs text-orange-700 mt-1 leading-relaxed">
                Untuk membeli paket membership, Anda <strong>wajib</strong> melengkapi data berikut di halaman profil:
              </p>
              <div className="flex flex-wrap gap-2 mt-2.5">
                {(!profile?.birthdate || profile.birthdate === "Belum diatur") ? (
                  <span className="inline-flex items-center gap-1 text-[11px] font-semibold bg-orange-100 text-orange-700 border border-orange-200 px-2.5 py-1 rounded-full">
                    <AlertTriangle className="w-3 h-3" /> Tanggal Lahir
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1 text-[11px] font-semibold bg-green-100 text-green-700 border border-green-200 px-2.5 py-1 rounded-full">
                    <CheckCircle className="w-3 h-3" /> Tanggal Lahir ✓
                  </span>
                )}
                {(!profile?.city || profile.city === "Belum diatur" || profile.city === "Kota Anda") ? (
                  <span className="inline-flex items-center gap-1 text-[11px] font-semibold bg-orange-100 text-orange-700 border border-orange-200 px-2.5 py-1 rounded-full">
                    <AlertTriangle className="w-3 h-3" /> Kota Domisili
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1 text-[11px] font-semibold bg-green-100 text-green-700 border border-green-200 px-2.5 py-1 rounded-full">
                    <CheckCircle className="w-3 h-3" /> Kota Domisili ✓
                  </span>
                )}
                {(!profile?.address || profile.address === "Belum diatur") ? (
                  <span className="inline-flex items-center gap-1 text-[11px] font-semibold bg-orange-100 text-orange-700 border border-orange-200 px-2.5 py-1 rounded-full">
                    <AlertTriangle className="w-3 h-3" /> Alamat Domisili
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1 text-[11px] font-semibold bg-green-100 text-green-700 border border-green-200 px-2.5 py-1 rounded-full">
                    <CheckCircle className="w-3 h-3" /> Alamat Domisili ✓
                  </span>
                )}
              </div>
            </div>
          ...
          <button
            onClick={() => router.push("/dashboard/profil")}
            className="flex items-center gap-2 px-5 py-2.5 bg-orange-600 hover:bg-orange-700 text-white text-xs font-bold rounded-xl transition-colors shadow-sm whitespace-nowrap flex-shrink-0"
                  ...
                  return (
                    ...
                    >
                      {purchasing === t.name ? (
                        ...
                        <>
                          {btnState.variant === "profile-incomplete" && <Lock className="w-3.5 h-3.5" />}
                          {btnState.text}
